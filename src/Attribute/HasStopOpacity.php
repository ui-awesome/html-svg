...
/**
 * Trait for managing the SVG `stop-opacity` attribute in tag rendering.
 *
 * Provides a method that delegates to `addAttribute()` to set the `stop-opacity` attribute on SVG elements.
 *
 * Intended for use in SVG tag and component classes that set gradient stop opacity.
 *
 * Key features.
 * - Delegates to `addAttribute()` for the `stop-opacity` attribute.
 * - Designed for use in SVG tag and component classes.
 * - Immutable method for setting or overriding the `stop-opacity` attribute.
 * - Supports `float`, `int`, `string`, and `null` for flexible stop opacity assignment (`0`-`1` range or unset).
 *
 * @method static addAttribute(string|UnitEnum $key, mixed $value) Adds an attribute and returns a new instance.
 * {@see \UIAwesome\Html\Mixin\HasAttributes} for managing attributes.
 *
 * @link https://developer.mozilla.org/en-US/docs/Web/SVG/Attribute/stop-opacity
 *
 * @copyright Copyright (C) 2026 Terabytesoftw.
 * @license https://opensource.org/license/bsd-3-clause BSD 3-Clause License.
 */
trait HasStopOpacity
{
    /**
     ...
     *
     * @param float|int|string|null $value Stop opacity value (for example, `0.5`, `'0.75'`, or `null` to unset).
     *
